...
from timestep_3d import  _Sim_Gma_gRidp_, _Sim_Xpv_gRidp_, _Sim_Xpv_gRidp_AB_, _update_Circulation
...
from remeshing_3d import _max_edge_length_dection_, _min_edge_length_dection_, _merging_parent_child_nodes_
from remeshing_3d import _creating_child_triangles_, _reassign_eleGma_on_added_tri_
from remeshing_3d import _delete_straddle_nodes_, _delete_straddle_elemenents_
from remeshing_3d import _delete_straddle_nodeVel_, _delete_straddle_nodeCirculation_, _delete_straddle_eleGma_
from remeshing_3d import _element_merging_below_critical_length_

#from remeshing_3d import _creating_child_triangles0_
...
def _init_field_creation( _imax_, _jmax_, _nodeXC, _nodeYC, _nodeZC, _no_tri_, tri_verti_ ):
...
def _Simulation_(Nx_vor, Ny_vor, x, y, At):
	...
	ds_max = 0.8*del_split
	ds_min = 0.5*del_split
	
	ds = 2.5*del_split
	
	print( 'del_split = ', del_split, ' ds_max = ', ds_max, ' ds_min = ', ds_min )
...
#	_del_split = 2.5*_edge_len_
	...
	_init_field_creation(Nx_vor, Ny_vor, _nodeXC, _nodeYC, _nodeZC, _no_tri_, tri_verti_)
...
#	sio.savemat('xleft.mat', {'xp':xp})
	...
	for Iter in range(Max_Iter):
		...
		if Iter%5 == 0 and Iter>0:
			...
			_tri_ele_max_edge_sorted_, _max_edge_sorted_ = _max_edge_length_dection_( _triXC, _triYC, _triZC, ds_max )
			#_tri_ele_min_edge_sorted_, _min_edge_sorted_ = _min_edge_length_dection_( _triXC, _triYC, _triZC, ds_min )
			
			_tri_ele_min_edge_sorted_ = []
			
			print( 'no of triangles have max edge length = ', len(_tri_ele_max_edge_sorted_) )
			print( 'no of triangles have min edge length = ', len(_tri_ele_min_edge_sorted_) )
			
			if len(_tri_ele_max_edge_sorted_) > 0:
				
				_len_ = len(_nodeXC)
				
				_nodeXC, _nodeYC, _nodeZC = \
				_merging_parent_child_nodes_(_triXC, _triYC, _triZC, _tri_ele_max_edge_sorted_, _max_edge_sorted_, _nodeXC, _nodeYC, _nodeZC)
				
				_triXC, _triYC, _triZC, tri_verti_ = \
				_creating_child_triangles_(_triXC, _triYC, _triZC, tri_verti_, _tri_ele_max_edge_sorted_, _max_edge_sorted_, _len_)
				
				tri_verti_ = tri_verti_.astype(int) 
				_no_tri_ = np.shape(tri_verti_)[0]
				print('_no_tri_ = ', _no_tri_)
				
				_eleGma = _reassign_eleGma_on_added_tri_( _eleGma, _tri_ele_max_edge_sorted_ )
			...
			if len(_tri_ele_min_edge_sorted_) > 0:
				
				_Triangle_Area_ = _allTriangle_Area_( _triXC,  _triYC,  _triZC,  _no_tri_, tri_verti_ )
				
				_triXC, _triYC, _triZC, tri_verti, _nodeXC, _nodeYC, _nodeZC, _eleGma = _element_merging_below_critical_length_\
				(_triXC, _triYC, _triZC, tri_verti_, _tri_ele_min_edge_sorted_, _min_edge_sorted_, _nodeXC, _nodeYC, _nodeZC, _Triangle_Area_, _eleGma)
				
				_no_tri_ = np.shape(Nw_tri_verti_)[0]
				print('_no_tri_ = ', _no_tri_)
			...
			if len(_tri_ele_max_edge_sorted_) == 0 and len(_tri_ele_min_edge_sorted_) == 0:
...
#			sio.savemat( 'trix%s.mat' %Iter, {'xt':_triXC} )
#			sio.savemat( 'triy%s.mat' %Iter, {'yt':_triYC} )
#			sio.savemat( 'triz%s.mat' %Iter, {'zt':_triZC} )
			sio.savemat('KE3d.mat', {'ke3d':KE3d} )
		

if __name__ == '__main__':
	Nx_vor = 60            # No of Lagrangian nodes in x-direction
	Ny_vor = 60            # No of Lagrangian nodes in y-direction
