...
def _MatrixInversion_2_cal_nodeCircu_frm_eleVor_( _nodeCoord, _triXC, _triYC, _triZC, _no_tri_, tri_verti_, _eleGma_, del_split ):
...
class _node_Circulation(object):
	...
	def __init__(self, _nodeCoord, _triXC, _triYC, _triZC, _no_tri_, tri_verti_, _eleGma_, del_split):
	...
	def _change_in_Circu(self):
		...
		(  self._nodeCoord, self._triXC, self._triYC, self._triZC, self._no_tri_, self.tri_verti_, self._eleGma_, self.del_split )
...
def _cal_nodeVelocity_frm_gridVelocity(_eleGma_, _no_tri_, tri_verti_, _nodeXC, _nodeYC, _nodeZC, _triXC, _triYC, _triZC):
	...
	#for i in range(len(x3d)):
	#	if x3d[i] == 0.5 and y3d[i] == 0.5 and z3d[i] == 0.: 
	#		print( _gridVelX_[i], _gridVelY_[i], _gridVelZ_[i] )
	
	shared_array_base = mp.Array( ctypes.c_double, len(_nodeXC)*3 )
	
	nprocs = 25 #mp.cpu_count()
	if len(_nodeXC)%nprocs != 0:
		raise ValueError ('Change number of processors to make len(_nodeXC)%nprocs = 0')
...
def _sum_in_parallel_B_( rank, nprocs, _gridVelX_, _gridVelY_, _gridVelZ_, _nodeXC, _nodeYC, _nodeZC, x3d, y3d, z3d, ds ):
	
	for itr in range( rank, len(_nodeXC), nprocs ):
		_nodeC = np.array( [ _nodeXC[itr], _nodeYC[itr], _nodeZC[itr] ] )
		sum_x, sum_y, sum_z = \
...
def _cal_eleVorStrgth_frm_nodeCirculation( _nodeTau_, _no_tri_, tri_verti_, _nodeXC, _nodeYC, _nodeZC, _triXC, _triYC, _triZC, del_split ):
