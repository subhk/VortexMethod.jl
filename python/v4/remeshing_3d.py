...
#			(_nodeXC, _nodeYC, _nodeZC, _triXC, _triYC, _triZC, _node_lt, _node_rt, _node_dn, _node_up, ds_min)
	...
	for ktr in range( len(_triXC[:,0]) ):
		p0 =  \
		np.intersect1d( np.where(_triXC[ktr,0] == _nodeXC)[0], np.where(_triYC[ktr,0] == _nodeYC)[0] )  #, np.where(_triZC[ktr,0] == _nodeZC)[0])
		p1 =  \
		np.intersect1d( np.where(_triXC[ktr,1] == _nodeXC)[0], np.where(_triYC[ktr,1] == _nodeYC)[0] ) #, np.where(_triZC[ktr,1] == _nodeZC)[0])
		p2 =  \
		np.intersect1d( np.where(_triXC[ktr,2] == _nodeXC)[0], np.where(_triYC[ktr,2] == _nodeYC)[0] )  #, np.where(_triZC[ktr,2] == _nodeZC)[0])
...
def _construct_elements_in_X_(_triXC, _triYC, _triZC, _eleGma, _nodeXC, _nodeYC, _nodeZC, Lx, ds_max, _ele_lt, _ele_rt):
	...
	for itr in range( len(_nodeXC) ):
		if _nodeXC[itr] < -ds_max and _nodeXC[itr] < 0: 
	...
	while len(p0) > 0:
		...
		for ktr in range( len(p0) ):
			ele_to_move = p0[ktr]
			_triXC, _triYC, _triZC, _ele_lt = \
			_moving_tri_ele_in_X_(_nodeXC[itrx], _nodeYC[itrx], _nodeZC[itrx], ele_to_move, _triXC, _triYC, _triZC, _nodeXC, _nodeYC, _nodeZC, Lx, _ele_lt)
...
################################################
	...
	for itr in range( len(_nodeXC) ):
		if _nodeXC[itr] > Lx+ds_max and _nodeXC[itr] > Lx: 
	...
	while len(p1) > 0:
		...
		for ktr in range( len(p1) ):
			ele_to_move = p1[ktr]
			_triXC, _triYC, _triZC, _ele_rt = \
			_moving_tri_ele_in_X_(_nodeXC[itrx], _nodeYC[itrx], _nodeZC[itrx], ele_to_move, _triXC, _triYC, _triZC, _nodeXC, _nodeYC, _nodeZC, -Lx, _ele_rt)
	...
	_triVt2 = []
	for ktr in range( len(_triXC[:,0]) ):
		p0 = np.intersect1d( np.where(_triXC[ktr,0] == _nodeXC)[0], np.where(_triYC[ktr,0] == _nodeYC)[0] )
		p1 = np.intersect1d( np.where(_triXC[ktr,1] == _nodeXC)[0], np.where(_triYC[ktr,1] == _nodeYC)[0] )
		p2 = np.intersect1d( np.where(_triXC[ktr,2] == _nodeXC)[0], np.where(_triYC[ktr,2] == _nodeYC)[0] )
	...
	return _triXC, _triYC, _triZC, _triVT0, _eleGma, _nodeXC, _nodeYC, _nodeZC #, flag #, _ele_lt, _ele_rt
	
	
def _construct_elements_in_Y_(_triXC, _triYC, _triZC, _eleGma, _nodeXC, _nodeYC, _nodeZC, Ly, ds_max, _ele_dn, _ele_up):
	...
	while len(p0) > 0:
		...
		for ktr in range( len(p0) ):
			ele_to_move = p0[ktr]
			_triXC, _triYC, _triZC, _ele_dn = \
			_moving_tri_ele_in_Y_(_nodeXC[itrx], _nodeYC[itrx], _nodeZC[itrx], ele_to_move, _triXC, _triYC, _triZC, _nodeXC, _nodeYC, _nodeZC, Ly, _ele_dn)
...
################################################
	...
	while len(p1) > 0:
		...
		for ktr in range( len(p1) ):
			ele_to_move = p1[ktr]
			_triXC, _triYC, _triZC, _ele_up = \
			_moving_tri_ele_in_Y_(_nodeXC[itrx], _nodeYC[itrx], _nodeZC[itrx], ele_to_move, _triXC, _triYC, _triZC, _nodeXC, _nodeYC, _nodeZC, -Ly, _ele_up)
	...
	return _triXC, _triYC, _triZC, _triVT0, _eleGma, _nodeXC, _nodeYC, _nodeZC #, flag
...
def _moving_tri_ele_in_X_(px0, py0, pz0, ele_to_move, _triXC, _triYC, _triZC, _nodeXC, _nodeYC, _nodeZC, lx, _tri_ele_):
	...
	if _edge_ == 0:
		dx0 = abs( _triXC[ele_to_move,1] - _triXC[ele_to_move,0] )
		dx1 = abs( _triXC[ele_to_move,2] - _triXC[ele_to_move,0] )
		
		_triXC[ele_to_move, 0] += lx
		...
		if lx < 0.: 
			_triXC[ele_to_move,1] = _triXC[ele_to_move,0] - dx0
			_triXC[ele_to_move,2] = _triXC[ele_to_move,0] - dx1
		if lx > 0.: 
			_triXC[ele_to_move,1] = _triXC[ele_to_move,0] + dx0
			_triXC[ele_to_move,2] = _triXC[ele_to_move,0] + dx1
		
	if _edge_ == 1:
		dx0 = abs( _triXC[ele_to_move,0] - _triXC[ele_to_move,1] )
		dx1 = abs( _triXC[ele_to_move,2] - _triXC[ele_to_move,1] )
		
		_triXC[ele_to_move, 1] += lx
		...
		if lx < 0.:
			_triXC[ele_to_move,0] = _triXC[ele_to_move,1] - dx0
			_triXC[ele_to_move,2] = _triXC[ele_to_move,1] - dx1
		if lx > 0.:
			_triXC[ele_to_move,0] = _triXC[ele_to_move,1] + dx0
			_triXC[ele_to_move,2] = _triXC[ele_to_move,1] + dx1
		
	if _edge_ == 2:
		dx0 = abs( _triXC[ele_to_move,0] - _triXC[ele_to_move,2] )
		dx1 = abs( _triXC[ele_to_move,1] - _triXC[ele_to_move,2] )
		
		_triXC[ele_to_move, 2] += lx
		...
		if lx < 0:
			_triXC[ele_to_move,0] = _triXC[ele_to_move, 2] - dx0
			_triXC[ele_to_move,1] = _triXC[ele_to_move, 2] - dx1
		if lx > 0:
			_triXC[ele_to_move,0] = _triXC[ele_to_move, 2] + dx0
			_triXC[ele_to_move,1] = _triXC[ele_to_move, 2] + dx1
	
#	p0 = np.intersect1d( np.where( _nodeXC[_nearest_node_] == _triXC)[0], np.where(_nodeYC[_nearest_node_] == _triYC)[0] )
#	indx0 = np.where( _ == _triXC )[0]
#	
#	for itr in range( len(_tri_ele0_) ):
#		if _tri_ele0_[itr] == p0:
#			_tri_ele_[itr] = ele_to_move
#			print('tri element at boundary has changed')
			
	return _triXC, _triYC, _triZC, _tri_ele_
	
	
def _moving_tri_ele_in_Y_(px0, py0, pz0, ele_to_move, _triXC, _triYC, _triZC, _nodeXC, _nodeYC, _nodeZC, ly, _tri_ele_):
	...
	if _edge_ == 0:
		dy0 = abs( _triYC[ele_to_move,1] - _triYC[ele_to_move,0] )
		dy1 = abs( _triYC[ele_to_move,2] - _triYC[ele_to_move,0] )
		
		_triYC[ele_to_move, 0] += ly 
		
		_nearest_node_ = _find_nearest_node_( _nodeXC, _nodeYC, _nodeZC, _triXC[ele_to_move,0], _triYC[ele_to_move,0], _triZC[ele_to_move,0] )
		(_triXC[ele_to_move,0], _triYC[ele_to_move,0], _triZC[ele_to_move,0]) = (_nodeXC[_nearest_node_], _nodeYC[_nearest_node_], _nodeZC[_nearest_node_])
		
		if ly < 0.:
			_triYC[ele_to_move,1] = _triYC[ele_to_move,0] - dy0
			_triYC[ele_to_move,2] = _triYC[ele_to_move,0] - dy1
		if ly > 0.:
			_triYC[ele_to_move,1] = _triYC[ele_to_move,0] + dy0
			_triYC[ele_to_move,2] = _triYC[ele_to_move,0] + dy1
		
	if _edge_ == 1:
		dy0 = abs( _triYC[ele_to_move,0] - _triYC[ele_to_move,1] )
		dy1 = abs( _triYC[ele_to_move,2] - _triYC[ele_to_move,1] )
		
		_triYC[ele_to_move, 1] += ly
		
		_nearest_node_ = _find_nearest_node_( _nodeXC, _nodeYC, _nodeZC, _triXC[ele_to_move,1], _triYC[ele_to_move,1], _triZC[ele_to_move,1] )
		(_triXC[ele_to_move,1], _triYC[ele_to_move,1], _triZC[ele_to_move,1]) = (_nodeXC[_nearest_node_], _nodeYC[_nearest_node_], _nodeZC[_nearest_node_])
		
		if ly < 0.:
			_triYC[ele_to_move,0] = _triYC[ele_to_move,1] - dy0
			_triYC[ele_to_move,2] = _triYC[ele_to_move,1] - dy1
		if ly > 0.:
			_triYC[ele_to_move,0] = _triYC[ele_to_move,1] + dy0
			_triYC[ele_to_move,2] = _triYC[ele_to_move,1] + dy1
		
	if _edge_ == 2:
		dy0 = abs( _triYC[ele_to_move,0] - _triYC[ele_to_move,2] )
		dy1 = abs( _triYC[ele_to_move,1] - _triYC[ele_to_move,2] )
		_triYC[ele_to_move, 2] += ly
		
		_nearest_node_ = _find_nearest_node_( _nodeXC, _nodeYC, _nodeZC, _triXC[ele_to_move,2], _triYC[ele_to_move,2], _triZC[ele_to_move,2] )
		(_triXC[ele_to_move,2], _triYC[ele_to_move,2], _triZC[ele_to_move,2]) = (_nodeXC[_nearest_node_], _nodeYC[_nearest_node_], _nodeZC[_nearest_node_])
		
		if ly < 0.:
			_triYC[ele_to_move,0] = _triYC[ele_to_move,2] - dy0
			_triYC[ele_to_move,1] = _triYC[ele_to_move,2] - dy1
		if ly > 0.:
			_triYC[ele_to_move,0] = _triYC[ele_to_move,2] + dy0
			_triYC[ele_to_move,1] = _triYC[ele_to_move,2] + dy1
	
#	p0 = np.intersect1d( np.where( _nodeXC[_nearest_node_] == _triXC)[0], np.where(_nodeYC[_nearest_node_] == _triYC)[0] )
#	for itr in range( len(_tri_ele_) ):
#		if _tri_ele_[itr] == p0:
#			_tri_ele_[itr] = ele_to_move
#			print('tri element at boundary has changed')	
