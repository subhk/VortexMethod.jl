...
def _Simulation_(Nx_vor, Ny_vor, x, y, At):
	...
	del_split = min( abs( x[1] - x[0] ), abs( y[1] - y[0] ) )
	
	ds_max = 3.5*del_split
	ds_min = 0.4*del_split
	...
	for Iter in range(Max_Iter):
		...
		if Iter%10 == 0 and Iter>0:
			...
			if len(_tri_ele_max_edge_sorted_) > 0:
				#ds_max = 1.1*ds_max 
				print( 'no of triangle to be modified = ', len(_tri_ele_max_edge_sorted_) )
...
if __name__ == '__main__':
	Nx_vor = 70           # No of Lagrangian nodes in x-direction
	Ny_vor = 140           # No of Lagrangian nodes in y-direction
